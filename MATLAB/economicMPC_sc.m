...
function [xt, yt, ut, t, total_cost] = economicMPC_sc(ssM, param_constraints, param_objective, param_simulation)
...
for i = 1:N-1
    ...
    objective = objective + c*sum(u(:,i)) + epsilon(:,i)'*S*epsilon(:,i); %economic cost function
end 
...
objective = objective + c*sum(u(:,N)) + epsilon(:,N)'*S*epsilon(:,N); %terminal economic cost function
